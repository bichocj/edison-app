...
class RestDatasource {
  ...
  Future<List<ClientCreditModel>> getClientCredit(String token, int clientId){
    return _netUtil.get(CLIENT_CREDIT_URL + clientId.toString(), token).then((dynamic res) {
      print(CLIENT_CREDIT_URL + clientId.toString());
      final itemsTmp = res.map((i) => new ClientCreditModel.map(i));
      final items = itemsTmp.cast<ClientCreditModel>();
      return items.toList();
    });
  }
}